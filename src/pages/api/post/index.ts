...
import { createPost, getPosts, deletePost, editPost } from '../../../modules/posts.service'
import { createPostSchema, deletePostSchema, editPostSchema } from '../../../modules/post.schema'
...
handler
  ...
  .patch(validation(editPostSchema), async (req: NextApiRequest, res: NextApiResponse) => {
    try{
      const {id, title, task} = req.query
      const refreshPost = await editPost(id, title, task)
      if(refreshPost)
        return res.status(200).send({ok: true})
